...
public:
  ...
  vector<TString> IterL3Objs;
	...
	Bool_t IsProbe( KPMuon probeCand, NtupleHandle *ntuple )
	{
		...
		if( probeCand.IsL1Matched(ntuple, 10) ) //FIXME pt cut? any more cuts?
			flag = kTRUE;
...
public:
  ...
  vector<TString> iterl3objs;
	...
	void Produce()
	{
		...
		tnpHist->SetPtThreshold( 10 ); // -- only probes above this pT cut will be filled in histograms -- // // FIXME Pt cut?
